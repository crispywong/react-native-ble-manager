...
public class Peripheral extends BluetoothGattCallback {
    ...
    if (!connected) {
        if (gatt != null) {
            try {
              gatt.disconnect();
              if (force) {
                Log.d(BleManager.LOG_TAG, "Force Disconnect");
                gatt.close();
                gatt = null;
  ...
  public void onConnectionStateChange(BluetoothGatt gatta, int status, int newState) {
    ...
    if (status != BluetoothGatt.GATT_SUCCESS) {
      if (newState == 0 && status == 133 || newState == 0 && status == 34 || newState == 0 && status == 8 || newState == 0 && status == 40) {
                                          Log.d(BleManager.LOG_TAG, "onConnectionStateChange require to reconnect");

        gatt.disconnect();
        gatt.close();
        gatt.getDevice().connectGatt(reactContext.getApplicationContext(), false, this);
        return;
      } else{
        gatt.close();
    ...
    if (newState == BluetoothProfile.STATE_CONNECTED) {
      ...
      if (gatt != null) {
          Log.d(BleManager.LOG_TAG, "onConnectionStateChange close gatt");
          gatt.close();
          gatt = null;
      }
  ...
  // private void setNotify(UUID serviceUUID, UUID characteristicUUID, final Boolean notify, Integer buffer,
  //      Callback callback) {
  ...
  //  Log.d(BleManager.LOG_TAG, "chars de prpop:" + String.valueOf(characteristic.getProperties()) + characteristic.getUuid().toString());
  ...
  public void write(UUID serviceUUID, UUID characteristicUUID, byte[] data, Integer maxByteSize, Integer queueSleepTime, Callback callback, int writeType) {
    ...
    if (data.length <= maxByteSize) {
      ...
      while (count < dataLength && (dataLength - count > maxByteSize)) {
        if (count == 0) {
          Log.d(BleManager.LOG_TAG, "lslsl0");
          firstMessage = Arrays.copyOfRange(data, count, count + maxByteSize);
          Log.d(BleManager.LOG_TAG, "fff" + bytesToHex(firstMessage));
        } else {
          Log.d(BleManager.LOG_TAG, "ekse");

          byte[] splitMessage = Arrays.copyOfRange(data, count, count + maxByteSize);
          Log.d(BleManager.LOG_TAG, "ddddd" + bytesToHex(splitMessage));
      ...
      if (BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT == writeType) {
        ...
        if (! doWrite(characteristic, firstMessage, callback)) {
          writeQueue.clear();
          writeCallback = null;
          callback.invoke("Write failed789");
        }
      } else {
          try {
            ...
            if (! doWrite(characteristic, firstMessage, callback)) {
              writeError = true;
              callback.invoke("Write failed456");
            }
            if (! writeError) {
              ...
              for (byte[] message : splittedMessage) {
                if (! doWrite(characteristic, message, callback)) {
                  writeError = true;
                  callback.invoke("Write failed123");
                  break;
                }
                Log.d("BLE", "send meeg1: " + bytesToHex(message));
                Thread.sleep(queueSleepTime);
              }
              if (! writeError) {
                callback.invoke();
              ...
              Log.d("BLE", "InterruptedException: " + e);
  ...
  //              Log.d(BleManager.LOG_TAG, "data length: " + String.valueOf(data.length) + " max: " + String.valueOf(maxByteSize));
  //              if (BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT == writeType) {
  //                  writeCallback = callback;
  //              }
  ...
  //                  while (count < dataLength && (dataLength - count > maxByteSize)) {
  //                      int chunkSize = dataLength - count > maxByteSize ? maxByteSize : dataLength - count;
  //                      Log.d(BleManager.LOG_TAG, "ssssfwefwefwef " +  String.valueOf(count));
  //                      if (count == 0) {
  //                          Log.d(BleManager.LOG_TAG, "lslsl0");
  //                          firstMessage = Arrays.copyOfRange(data, count, count + maxByteSize);
  //                          Log.d(BleManager.LOG_TAG, "fff" + bytesToHex(firstMessage));
  //                          // splittedMessage.add(firstMessage);

  //                      } else {
  //                          Log.d(BleManager.LOG_TAG, "ekse");

  //                          byte[] splitMessage = Arrays.copyOfRange(data, count, count + chunkSize);
  //                          Log.d(BleManager.LOG_TAG, "ddddd" + bytesToHex(splitMessage));
  ...
  //                      count += chunkSize;
  //                  }
  //                          Log.d(BleManager.LOG_TAG, "abcdd");
  ...
  //                      splittedMessage.add(splitMessage);
  //                  }
  //                  Log.d(BleManager.LOG_TAG, String.valueOf(splittedMessage.size()));

  //                  if (BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT == writeType) {
  //                      Log.d(BleManager.LOG_TAG, "dndjsjsjs");
  //                      writeQueue.addAll(splittedMessage);
  ...
  //                              Thread.sleep(queueSleepTime);
  //                              for (byte[] message : splittedMessage) {
  //                                  Log.d("BLE", "send meeg: " + bytesToHex(message));
